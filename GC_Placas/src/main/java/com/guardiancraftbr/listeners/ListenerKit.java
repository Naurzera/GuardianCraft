...
public class ListenerKit implements Listener {

    @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = false)
    public void onSignPlace(SignChangeEvent e) {
        if (e.getLine(0).toLowerCase().contains("[kit]")) {
            ...
            e.setLine(0, ChatColor.DARK_BLUE + "[Kit]");
        ...
        if (e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
            if (e.getClickedBlock().getType().toString().contains("SIGN")) {
                ...
                if (sign.getLine(0).contains("[Kit]")) {
                    ...
                    if (execute){
                        String darkit = CM.cmdkit.replaceAll("%player%",e.getPlayer().getName())
                                .replaceAll("%kit%", sign.getLine(1));
                        System.out.println("[GC_Placas] Console executando: "+darkit);
                        Main.getInstance().getServer().dispatchCommand(Bukkit.getConsoleSender(), darkit);
                        e.getPlayer().sendMessage(CM.msgrecebeukit.replaceAll("%kit%", sign.getLine(1)));
                    } else {
