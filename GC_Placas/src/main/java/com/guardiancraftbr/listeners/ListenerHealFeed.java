...
public class ListenerHealFeed implements Listener {

    @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = false)
    public void onSignPlace(SignChangeEvent e) {
        if (e.getLine(0).toLowerCase().contains("[heal]")) {
        ...
        if (e.getLine(0).toLowerCase().contains("[feed]")) {
            if (!e.getPlayer().hasPermission(CM.staffperm)){
                e.setCancelled(true);
                e.getBlock().breakNaturally();
                e.getPlayer().sendMessage(ChatColor.RED+"Você não tem permissão para fazer isso.");
            ...
            if (e.getClickedBlock().getType().toString().contains("SIGN")) {
                ...
                if (sign.getLine(0).contains("[Heal]")) {
                    ...
                    if (execute){
                        ...
                        e.getPlayer().sendMessage(CM.msgheal);
                    } else {
                        e.getPlayer().sendMessage(CM.msgmoney);
                        e.setCancelled(true);
                    }
                }
                if (sign.getLine(0).contains("[Feed]")) {
                    if (Cooldown.isInCooldown(e.getPlayer().getUniqueId(), "placas")) return;
                    Cooldown c = new Cooldown(e.getPlayer().getUniqueId(), "placas", 1);
                    c.start();
                    boolean execute = true;
                    ...
                    if (execute){
                        ...
                        e.getPlayer().sendMessage(CM.msgfeed);
                    } else {
                        e.getPlayer().sendMessage(CM.msgmoney);
                        e.setCancelled(true);
                    }
